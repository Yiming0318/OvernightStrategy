...
def get_data(ticker1, ticker2, start_date, end_date, feature1, feature2, rolling_window):
    ...
    df2 = pd.DataFrame(columns= [ticker1, ticker2, feature2])
    df[ticker1] = data_1_1
    df[ticker2] = data_2
    df[feature2] = data_1_2
    # df['Days'] = range(1, days+1)
    # standard deviation
    ...
    # print(df.tail(30))
    global TODAY_DATA 
    TODAY_DATA = normalize(df, ticker1)[-1]
    global TODAY_PRICE 
    TODAY_PRICE = df.iloc[-1].tolist()[0]
    print("{} TODAY PRICE: {}".format(ticker1, TODAY_PRICE))
    global TEST_DATA
    TEST_DATA = normalize(df, ticker1)[-TEST_SIZE:]
    global TEST_ACTUAL_PRICE
    TEST_ACTUAL_PRICE = df[ticker1][-TEST_SIZE:]
    # print(TEST_ACTUAL_PRICE)
    # print(len(TEST_ACTUAL_PRICE))
    # print(len(TEST_DATA))
    # print(TODAY_DATA)
    ...
    # print("##############")
    # print("Delay One Day")
    df2[ticker1] = df[ticker1].shift(-1)
    df2[ticker2] = df[ticker2]
    df2[feature2] = df[feature2]
    df2["STD"] = df["STD"]
    df2["MA"] = df["MA"]
...
def make_multi_linear_regression(x_train, x_test, y_train, y_test):
    ...
    # draw_true_predict(TEST_ACTUAL_PRICE, test_predict)
...
def make_random_forest_regressor(x_train, x_test, y_train, y_test):
    ...
    r_2 = r2_score(y_test, y_predict)
    print("The r^2 for random forest regressor regression is", r_2)
...
# make random forest regressor
def make_svm_regressor(x_train, x_test, y_train, y_test):
    ...
    r_2 = r2_score(y_test, y_predict)
    print("The r^2 for random forest regressor regression is", r_2)
    tmr_price = svm_regr.predict([TODAY_DATA])
    test_predict = svm_regr.predict(TEST_DATA)
    # draw_true_predict(TEST_ACTUAL_PRICE, test_predict)
    print("tomorrow price is {}".format(tmr_price))
    if tmr_price - TODAY_PRICE >= 0:
      print("Recommendation:Long")
...
def draw_true_predict(true, predict, p=0):
